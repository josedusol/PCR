...
VARIABLES N, map1, map2, ip1

INSTANCE TLAPS
INSTANCE MainPCRFibPrimes2

PCRIsPrime == INSTANCE PCRIsPrimeThm

USE DEF Undef, IndexType1, IndexType2, CtxIdType1, CtxIdType2 
...
LEMMA Lem_BasicFunType == 
  ...
  BY Lem_fibType, Lem_sumType, Lem_divisorsType, Lem_notDividesType, Lem_andType
...
<1>1. Init => TypeInv
  ...
  <2>3. map2 \in PCR2!CtxMap
    BY DEF Init, PCR2!CtxMap  
  <2>4. ip1  \in PCR1!IndexMap
...
<1>2. TypeInv /\ IteratorType /\ [Next]_vars => TypeInv'
  ...
  <2>1. CASE \E I \in Seq(Nat) : Next1(I)
    ...
    <3>6. map2  \in PCR2!CtxMap       BY <2>0 DEF TypeInv            
    <3>A. CASE /\ PCR1!state(I) = "OFF"
               /\ PCR1!Start(I)
               /\ UNCHANGED <<map2,ip1>>   
      <4>1. map2' \in PCR2!CtxMap     BY <2>0, <3>A DEF TypeInv
      <4>2. ip1'  \in PCR1!IndexMap   BY <2>0, <3>A DEF TypeInv
      <4>3. map1' \in PCR1!CtxMap 
    ...
    <3>B. CASE /\ PCR1!state(I) = "RUN"
               /\ PCR1!P(I)   
               /\ UNCHANGED <<map2>>
      <4>1. map2' \in PCR2!CtxMap     BY <2>0, <3>B DEF TypeInv 
    ...
    <3>C. CASE /\ PCR1!state(I) = "RUN"
               /\ PCR1!C_call(I)
               /\ UNCHANGED ip1
      <4>1. ip1'  \in PCR1!IndexMap   BY <2>0, <3>C DEF TypeInv   
      <4>2. map1' \in PCR1!CtxMap
      <4>3. map2' \in PCR2!CtxMap
      <4> QED 
        BY <3>1, <4>1, <4>2, <4>3 DEF TypeInv                    
    <3>D. CASE /\ PCR1!state(I) = "RUN"
               /\ PCR1!C_ret(I)        
               /\ UNCHANGED <<map2,ip1>> 
      <4>1. map2' \in PCR2!CtxMap     BY <2>0, <3>D DEF TypeInv  
      <4>2. ip1'  \in PCR1!IndexMap   BY <2>0, <3>D DEF TypeInv   
      <4>3. map1' \in PCR1!CtxMap
        ...
        <5>3. map1[I].v_c' \in PCR1!VarC
          <6>1. map1[I].v_c' = [map1[I].v_c EXCEPT 
                  ![i] = [v |-> PCR1!isPrime!out(I \o <<i>>), 
                          r |-> 0]] 
            BY <3>2, <5>1 DEF PCR1!CtxMap, PCR1!Ctx
          <6>2. map1[I].v_c[i]' = [v |-> PCR1!isPrime!out(I \o <<i>>), 
                                   r |-> 0] 
            BY <3>4, <5>2, <6>1 DEF PCR1!Ctx, PCR1!VarC
          <6>3. PCR1!isPrime!out(I \o <<i>>) \in VarCType1 
\*            <7>1. PCR1!Null = Null
\*              BY DEF PCR1!Null, Null
            <7>2. PCR2!Undef = PCR1!isPrime!Undef 
              BY DEF PCR2!Undef, PCR1!isPrime!Undef, PCR1!IndexType2, PCR1!VarPType2, VarPType2,
                 PCR1!VarCType2, VarCType2, PCR1!isPrime!State, PCR2!State, PCR1!VarRType2,
                 VarRType2, PCR1!InType2, InType2
            <7>3. I \o <<i>> \in Seq(Nat) BY <5>2
            <7>4. map2[I \o <<i>>] # PCR2!Undef            
              BY <5>1, <7>2 DEF PCR1!isPrime!wellDef 
            <7>5. map2[I \o <<i>>] \in PCR2!Ctx        
              BY <3>6, <7>3, <7>4 DEF PCR2!CtxMap, PCR2!Ctx        
            <7> QED 
              BY <7>5 DEF PCR1!isPrime!out, PCR2!Ctx, VarRType2, VarCType1
          <6> QED
    ...
    <3>E. CASE /\ PCR1!state(I) = "RUN"
               /\ PCR1!R(I) 
               /\ UNCHANGED <<map2,ip1>>
      <4>1. map2' \in PCR2!CtxMap     BY <2>0, <3>E DEF Next2, TypeInv  
      <4>2. ip1'  \in PCR1!IndexMap   BY <2>0, <3>E DEF Next2, TypeInv   
      <4>3. map1' \in PCR1!CtxMap
        <5>1. PICK i \in PCR1!iterator(I) : 
        ...
        <5>3. map1[I].ret' \in VarRType1
          <6>1. map1[I].ret' = PCR1!sum(map1[I].ret, PCR1!v_c(I)[i].v) 
            BY <3>2, <5>1 DEF PCR1!CtxMap, PCR1!Ctx
          <6>2. PCR1!sum(map1[I].ret, PCR1!v_c(I)[i].v) \in VarRType1 
            BY Lem_BasicFunType 
          <6> QED BY <6>1, <6>2
        <5>4. map1[I].v_c' \in PCR1!VarC
          <6>1. map1[I].v_c' = [map1[I].v_c EXCEPT 
                                  ![i] = [map1[I].v_c[i] EXCEPT 
                                            !.r = map1[I].v_c[i].r + 1]] 
            BY <3>2, <5>1 DEF PCR1!CtxMap, PCR1!Ctx
          <6>2. map1[I].v_c[i] # Undef 
            BY <5>1, Undef = PCR1!Undef DEF PCR1!written, PCR1!v_c          
          <6>3. map1[I].v_c[i].r' = map1[I].v_c[i].r + 1  
            BY <3>4, <5>2, <6>1 DEF PCR1!Ctx, PCR1!VarC        
          <6>4. map1[I].v_c[i].r \in Nat 
            BY <3>4, <5>2, <6>2 DEF PCR1!CtxMap, PCR1!Ctx, PCR1!VarC
          <6>5. map1[I].v_c[i].r' \in Nat 
            BY <6>3, <6>4
          <6> QED
            BY <3>2, <3>4, <5>2, <6>1, <6>2, <6>5 DEF PCR1!CtxMap, PCR1!Ctx, PCR1!VarC
        <5>5. map1[I].ste' \in PCR1!State
    ...
    <3>F. CASE /\ PCR1!state(I) = "RUN"
               /\ PCR1!Eureka(I) 
               /\ UNCHANGED <<map2,ip1>>
      <4>1. map2' \in PCR2!CtxMap     BY <2>0, <3>F DEF TypeInv  
      <4>2. ip1'  \in PCR1!IndexMap   BY <2>0, <3>F DEF TypeInv   
      <4>3. map1' \in PCR1!CtxMap   
        <5>1. map1' = [map1 EXCEPT ![I].ste = "END"] 
          BY <3>F DEF PCR1!Eureka
        <5>2. map1[I].ste' \in PCR1!State 
          BY <3>4, <5>1 DEF PCR1!State, PCR1!Ctx     
        <5>3. map1[I]' \in PCR1!Ctx 
          BY <3>3, <3>4, <5>1, <5>2 DEF PCR1!Ctx
        <5> QED 
          BY <3>2, <5>1, <5>3 DEF PCR1!Ctx, PCR1!CtxMap      
      <4> QED   
        BY <3>1, <4>1, <4>2, <4>3 DEF TypeInv                           
    <3>G. CASE /\ PCR1!state(I) = "RUN"
               /\ PCR1!Quit(I) 
               /\ UNCHANGED <<map2,ip1>>
      <4>1. map2' \in PCR2!CtxMap     BY <2>0, <3>G DEF TypeInv  
      <4>2. ip1'  \in PCR1!IndexMap   BY <2>0, <3>G DEF TypeInv   
      <4>3. map1' \in PCR1!CtxMap  
        <5>1. map1' = [map1 EXCEPT ![I].ste = "END"] 
          BY <3>G DEF PCR1!Quit
        <5>2. map1[I].ste' \in PCR1!State 
          BY <3>4, <5>1 DEF PCR1!State, PCR1!Ctx     
        <5>3. map1[I]' \in PCR1!Ctx 
          BY <3>3, <3>4, <5>1, <5>2 DEF PCR1!Ctx
      ...
      <4> QED   
        BY <3>1, <4>1, <4>2, <4>3 DEF TypeInv                                        
    <3> QED
      BY <3>0, <3>A, <3>B, <3>C, <3>D, <3>E, <3>F, <3>G DEF Next1, PCR1!Next, PCR1!C
  <2>2. CASE \E I \in Seq(Nat) : Next2(I)
    ...
    <3>2. map1' \in PCR1!CtxMap     BY <2>0, <3>0 DEF Next2, TypeInv  
    <3>3. ip1'  \in PCR1!IndexMap   BY <2>0, <3>0 DEF Next2, TypeInv    
    <3>4. map2  \in PCR2!CtxMap     BY <2>0 DEF TypeInv
    <3>5. map2[I] # Undef           BY <3>0 DEF Next2
    <3>6. map2[I] \in PCR2!Ctx      BY <3>4, <3>5 DEF PCR2!CtxMap        
    <3>A. CASE /\ PCR2!state(I) = "OFF"
               /\ PCR2!Start(I) 
      <4>1. map2' = [map2 EXCEPT ![I].ste = "RUN"] BY <3>A DEF PCR2!Start
      <4>2. map2[I].ste' \in PCR2!State BY <3>6, <4>1 DEF PCR2!State, PCR2!Ctx     
      <4>3. map2[I]' \in PCR2!Ctx BY <3>5, <3>6, <4>1, <4>2 DEF PCR2!Ctx  
      <4>4. map2' \in PCR2!CtxMap BY <3>4, <4>1, <4>3 DEF PCR2!Ctx, PCR2!CtxMap  
      <4> QED 
    ...
    <3>B. CASE /\ PCR2!state(I) = "RUN"
      ...
      <4>3. map2[I].v_p' \in PCR2!VarP
        <5>1. map2[I].v_p' = [map2[I].v_p EXCEPT 
    ...
    <3>C. CASE /\ PCR2!state(I) = "RUN"
      ...
      <4>3. map2[I].v_p' \in PCR2!VarP 
        <5>1. map2[I].v_p' = [map2[I].v_p EXCEPT 
                                ![i] = [map2[I].v_p[i] EXCEPT !.r = 1]] 
          BY <3>4, <4>1 DEF PCR2!CtxMap, PCR2!Ctx
        <5>2. map2[I].v_p[i] # Undef 
          BY <4>1, Undef = PCR2!Undef DEF PCR2!written, PCR2!v_p          
        <5>3. map2[I].v_p[i].r' = 1 
          BY <3>6, <4>2, <5>1 DEF PCR2!Ctx, PCR2!VarP
        <5>4. map2[I].v_p[i].r' \in Nat 
          BY <5>3
        <5> QED 
          BY <3>6, <4>2, <5>1, <5>2, <5>4 DEF PCR2!Ctx, PCR2!VarP        
      <4>4. map2[I].v_c' \in PCR2!VarC
        <5>1. map2[I].v_c' = [map2[I].v_c EXCEPT 
                                ![i] = [v |-> PCR2!notDivides(PCR2!in(I), PCR2!v_p(I), i), 
                                        r |-> 0]] 
          BY <3>4, <4>1 DEF PCR2!CtxMap, PCR2!Ctx
        <5>2. map2[I].v_c[i]' = [v |-> PCR2!notDivides(PCR2!in(I), PCR2!v_p(I), i), 
                                 r |-> 0] 
    ...
    <3>D. CASE /\ PCR2!state(I) = "RUN"
      ...
      <4>1. PICK i \in PCR2!iterator(I) : 
              ...
              /\ map2' = [map2 EXCEPT 
                   ![I].ret      = PCR2!and(map2[I].ret, PCR2!v_c(I)[i].v),
                   ![I].v_c[i].r = map2[I].v_c[i].r + 1,
                   ![I].ste      = IF PCR2!cDone(I, i) THEN "END" ELSE map2[I].ste]
        BY <3>D DEF PCR2!R
      <4>2. i \in Nat  
        BY <2>0, <3>5 DEF PCR2!CtxIndex, IteratorType    
      <4>3. map2[I].ret' \in VarRType2       
        <5>1. map2[I].ret' = PCR2!and(map2[I].ret, PCR2!v_c(I)[i].v) 
          BY <3>4, <4>1 DEF PCR2!CtxMap, PCR2!Ctx
        <5>2. PCR2!and(map2[I].ret, PCR2!v_c(I)[i].v) \in VarRType2 
          BY Lem_BasicFunType
        <5> QED 
          BY <5>1, <5>2
      <4>4. map2[I].v_c' \in PCR2!VarC    
        <5>1. map2[I].v_c' = [map2[I].v_c EXCEPT 
                                ![i] = [map2[I].v_c[i] EXCEPT 
                                          !.r = map2[I].v_c[i].r + 1]] 
          BY <3>4, <4>1 DEF PCR2!CtxMap, PCR2!Ctx  
        <5>2. map2[I].v_c[i] # Undef 
          BY <4>1, Undef = PCR2!Undef DEF PCR2!written, PCR2!v_c                
        <5>3. map2[I].v_c[i].r' = map2[I].v_c[i].r + 1  
          BY <3>6, <4>2, <5>1 DEF PCR2!Ctx, PCR2!VarC        
        <5>4. map2[I].v_c[i].r \in Nat 
          BY <3>6, <4>2, <5>2 DEF PCR2!CtxMap, PCR2!Ctx, PCR2!VarC
        <5>5. map2[I].v_c[i].r' \in Nat 
          BY <5>3, <5>4
        <5> QED
          BY <3>4, <3>6, <4>2, <5>1, <5>2, <5>5 DEF PCR2!CtxMap, PCR2!Ctx, PCR2!VarC
      <4>5. map2[I].ste' \in PCR2!State 
        <5>1. map2[I].ste' = IF PCR2!cDone(I, i) THEN "END" ELSE map2[I].ste 
          BY <3>4, <4>1 DEF PCR2!CtxMap, PCR2!Ctx
        <5>A. CASE PCR2!cDone(I, i) 
          <6>1. map2[I].ste' = "END" BY <5>1, <5>A
          <6>2. "END" \in PCR2!State BY DEF PCR2!State           
          <6> QED BY <6>1, <6>2
        <5>B. CASE ~ PCR2!cDone(I, i) 
          <6>1. map2[I].ste' = map2[I].ste BY <5>1, <5>B
          <6>2. map2[I].ste \in PCR2!State BY <3>6 DEF PCR2!CtxMap, PCR2!Ctx         
          <6> QED BY <6>1, <6>2          
        <5> QED 
          BY <5>A, <5>B               
      <4>6. map2[I]' \in PCR2!Ctx
        BY <3>4, <3>5, <3>6, <4>1, <4>3, <4>4, <4>5 DEF PCR2!CtxMap, PCR2!Ctx
      <4>7. map2' \in PCR2!CtxMap 
        BY <3>4, <4>1, <4>6 DEF PCR2!CtxMap, PCR2!Ctx
      <4> QED 
        BY <3>1, <3>2, <3>3, <4>7 DEF TypeInv                                              
    <3>E. CASE /\ PCR2!state(I) = "RUN"
               /\ PCR2!Eureka(I)    
      <4>1. map2' = [map2 EXCEPT ![I].ste = "END"] BY <3>E DEF PCR2!Eureka
      <4>2. map2[I].ste' \in PCR2!State BY <3>6, <4>1 DEF PCR2!State, PCR2!Ctx     
      <4>3. map2[I]' \in PCR2!Ctx BY <3>5, <3>6, <4>1, <4>2 DEF PCR2!Ctx
      <4>4. map2' \in PCR2!CtxMap BY <3>4, <4>1, <4>3 DEF PCR2!Ctx, PCR2!CtxMap                 
      <4> QED 
        BY <3>1, <3>2, <3>3, <4>4 DEF TypeInv                       
    <3>F. CASE /\ PCR2!state(I) = "RUN"
               /\ PCR2!Quit(I)                                        
      <4>1. map2' = [map2 EXCEPT ![I].ste = "END"] BY <3>F DEF PCR2!Quit
      <4>2. map2[I].ste' \in PCR2!State BY <3>6, <4>1 DEF PCR2!State, PCR2!Ctx     
      <4>3. map2[I]' \in PCR2!Ctx BY <3>5, <3>6, <4>1, <4>2 DEF PCR2!Ctx
      <4>4. map2' \in PCR2!CtxMap BY <3>4, <4>1, <4>3 DEF PCR2!Ctx, PCR2!CtxMap                 
      <4> QED 
        BY <3>1, <3>2, <3>3, <4>4 DEF TypeInv                                                                           
    <3> QED
      BY <3>0, <3>A, <3>B, <3>C, <3>D, <3>E, <3>F DEF Next2, PCR2!Next  
  <2>3. CASE Done
    <3>1. /\ N'    \in InType1
          /\ map1' \in PCR1!CtxMap  
          /\ map2' \in PCR2!CtxMap  
          /\ ip1'  \in PCR1!IndexMap  BY <2>0, <2>3 DEF Done, TypeInv, vars
    <3> QED 
      BY <3>1 DEF TypeInv  
  <2>4. CASE UNCHANGED vars
    <3>1. /\ N'    \in InType1
          /\ map1' \in PCR1!CtxMap  
          /\ map2' \in PCR2!CtxMap  
          /\ ip1'  \in PCR1!IndexMap  BY <2>0, <2>4 DEF TypeInv, vars  
    <3> QED 
      BY <3>1 DEF TypeInv
  <2> QED
...
\* Last modified Wed Oct 21 20:10:54 UYT 2020 by josedu
