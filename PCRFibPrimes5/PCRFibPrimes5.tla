...
P_call(i) == 
  \E j \in Iterator(i):
    /\ ~ Written(v_p(i), j)
    /\ ~ fibRec!WellDef(i \o <<j>>)
    /\ map2' = [map2 EXCEPT 
         ![i \o <<j>>] = fibRec!InitCtx(j)]    
\*    /\ PrintT("P_call" \o ToString(i \o <<j>>) 
\*                       \o " : in= " \o ToString(j))
...
P_ret(i) == 
  \E j \in Iterator(i) :
     /\ ~ Written(v_p(i), j)   
     /\ fibRec!WellDef(i \o <<j>>) 
     /\ fibRec!Finished(i \o <<j>>)   
     /\ map' = [map EXCEPT 
          ![i].v_p[j]= [v |-> fibRec!Out(i \o <<j>>), r |-> 0]]  
\*     /\ PrintT("P_ret" \o ToString(i \o <<j>>) 
\*                       \o " : in= "  \o ToString(fibRec!in(i \o <<j>>))    
\*                       \o " : ret= " \o ToString(fibRec!Out(i \o <<j>>)))
...
\* Last modified Fri Sep 25 02:40:03 UYT 2020 by josedu
