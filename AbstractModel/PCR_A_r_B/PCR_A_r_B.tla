...
AXIOM H_BFunTypeA ==
  \A x \in T, i \in AssigA :
    ...
    /\ \A vp \in StA(Tp1) : fc1(x,vp,i) \in Tc1 \union {Undef}
    /\ \A vc \in StA(Tc1) : fr1(x,vc,i) \in D   \union {Undef}

AXIOM H_BFunWDA ==
  \A x \in T : \A i \in ItA(x) : 
    /\ gp1(x,i) \in Tp1
    /\ \A vp \in StA(Tp1) : wrts(vp,depsA(x,Dep_pp1,i)\{i}) => fp1(x,vp,i) \in Tp1
    /\ \A vp \in StA(Tp1) : wrts(vp,depsA(x,Dep_pc1,i))     => fc1(x,vp,i) \in Tc1
    /\ \A vc \in StA(Tc1) : wrts(vc,depsA(x,Dep_cr1,i))     => fr1(x,vc,i) \in D  
...
AXIOM H_fcRelevanceA == 
  \A x \in T : \A i \in ItA(x), vp1 \in StA(Tp1), vp2 \in StA(Tp1) :
    eqs(vp1,vp2,depsA(x,Dep_pc1,i)) => fc1(x,vp1,i) = fc1(x,vp2,i)

AXIOM H_frRelevanceA ==
  \A x \in T : \A i \in ItA(x), vc1 \in StA(Tc1), vc2 \in StA(Tc1) :
...
AXIOM H_BFunTypeB ==
  \A x \in T2, i \in AssigB :
    ...
    /\ \A vc \in StB(Tc2) : fr2(x,vc,i) \in D   \union {Undef}
...
AXIOM H_BFunWDB ==
  \A x \in T2 : \A i \in ItB(x) : 
    ...
    /\ \A vc \in StB(Tc2) : wrts(vc,depsB(x,Dep_cr2,i))     => fr2(x,vc,i) \in D  
...
AXIOM H_AlgebraB == AbelianMonoid(D, id, Op2)
...
Fc1(x,vp) == [i \in AssigA |-> fc1(x,vp,i)] 
...
C1(I,i) == 
  ...
  /\ c1' = [c1 EXCEPT ![I][i] = fc1(X1[I],p1[I],i)]
...
R1ini(I,i) == 
  ...
  /\ ~ \E k \in ItA(X1[I]) : /\ k # i
                             /\ wrt(X2[I \o <<k>>])
                             /\ ~ redA(I,k)            
  /\ X2' = [X2 EXCEPT ![I \o <<i>>] = <<r1[I], fr1(X1[I],c1[I],i)>>]
...
CInvA == 
  \A i \in ItA(X1[I0]) :
    wrt(c1[I0][i]) => /\ wrts(p1[I0],depsA(X2[I0],Dep_pc2,i))
                      /\ c1[I0][i] = fc1(X1[I0],p1[I0],i)
...
RInv1B == 
  \A I \in WDIndexB : \A i \in ItB(X2[I]) : 
    redB(I0,i) => wrts(c2[I],depsB(X2[I],Dep_cr2,i))

RInv2B == 
  \A I \in WDIndexB :
    r2[I] = M2!BigOpP(lBnd2(X2[I]), uBnd2(X2[I]),
                      LAMBDA j : prop2(j) /\ redB(I0,j), 
...
PCR_A == INSTANCE PCR_A 
  WITH X <- X1, p <- p1, c <- c1, r <- r1, rs <- rs1,
       T <- T, Tp <- Tp1, Tc <- Tc1, D <- D, 
       id <- id, Op <- Op1,
       lBnd <- lBnd1, uBnd <- uBnd1, prop <- prop1,                 
       fp <- fp1, fc <- fc1, fr <- fr1, gp <- gp1,       
