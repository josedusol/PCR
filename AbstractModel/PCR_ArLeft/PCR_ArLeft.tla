...
A(x) == M!BigOp(lBnd(x), uBnd(x), LAMBDA i : Fr(x,Fc(x,Gp(x)))[i])
...
R(I,i) == 
  ...
  /\ i-1 >= lBnd(X[I]) => red(I,i-1)    \* dep r(i-1) -> r(i)
...
RInv1 == 
  \A i \in It(X[I0]) : 
    red(I0,i) => /\ wrts(c[I0],deps(X[I0],Dep_cr,i))
                 /\ \A k \in It(X[I0]) : k < i => red(I0,k)

RInv2 == 
  \A i \in It(X[I0]) : 
    ~ red(I0,i) => r[I0] = M!BigOpP(lBnd(X[I0]), i-1,
                                    LAMBDA j : red(I0,j),  
                                    LAMBDA j : fr(X[I0],c[I0],j))
...
PCR_A == INSTANCE PCR_A 
  WITH in <- inS, X <- X, p <- p, c <- c, r <- r, rs <- rs,
       T <- T, Tp <- Tp, Tc <- Tc, D <- D, 
       id <- id, Op <- Op,
       lBnd <- lBnd, uBnd <- uBnd, prop <- propS,                   
       fp <- fp, fc <- fc, fr <- fr, gp <- gp,       
       Dep_pp <- Dep_pp, Dep_pc <- Dep_pc, Dep_cr <- Dep_cr

============================================================================
\* Modification History
\* Last modified Wed Sep 08 18:29:13 UYT 2021 by josedu
\* Last modified Wed Jul 07 19:16:42 GMT-03:00 2021 by JosEdu
