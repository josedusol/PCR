...
\*          <6>1a. map1[i].v_p[map1[i].i_p] \in [v:VarPType1\cup{NULL}, r:Nat] BY <5>2, <3>3 DEF PCR1!CtxType, PCR1!VarP
          <6>1b. map1[i].v_p \in PCR1!VarP BY <3>3 DEF PCR1!VarP, PCR1!CtxType
          
\*          <6> DEFINE j == map1[i].i_p
\*          <6>2. map1[i].v_p' = [map1[i].v_p EXCEPT ![j] = [v |-> PCR1!fib(PCR1!in(i), PCR1!v_p(i), j), r |-> 0]] 
\*            BY <6>1
...
\*          <6>2. map1[i].i_p # NULL
          <6>5. map1[i].v_p[j]' = [v |-> PCR1!fib(PCR1!in(i), PCR1!v_p(i), j), r |-> 0]
            BY <6>1b, <6>2, <6>1, <6>1c DEF PCR1!CtxType, PCR1!VarP
            
          <6> QED
        <5>5. map1[i]' \in PCR1!CtxType BY <3>1, <3>2, <5>1, <5>3, <5>4 DEF PCR1!CtxMap, PCR1!CtxType, PCR1!VarP
        <5> QED 
          BY <3>1, <5>1, <5>5 DEF PCR1!CtxType, PCR1!CtxMap            
      <4> QED BY <4>1, <4>2 DEF TypeInv               
    <3>C. CASE /\ PCR1!Running(i) 
               /\ PCR1!C(i)
      <4>1. N' \in InType1 BY <2>0, <3>0 DEF TypeInv, Next1     
      <4>2. map1' \in PCR1!CtxMap 
        ...
        <5>3. map1[i].v_p' \in PCR1!VarP 
          <6>1. map1[i].v_p' = [map1[i].v_p EXCEPT ![j] = [map1[i].v_p[j] EXCEPT !.r = 1]] BY <3>1, <5>1 DEF PCR1!CtxMap, PCR1!CtxType
          <6>2. map1[i].v_p[j].r' = 1 BY <3>3, <5>2, <6>1 DEF PCR1!CtxType, PCR1!VarP
          <6>3. map1[i].v_p[j].r' \in Nat BY <6>2
          <6> QED 
            BY <3>3, <5>2, <6>1, <6>3 DEF PCR1!CtxType, PCR1!VarP        
        <5>4. map1[i].v_c' \in PCR1!VarC
          <6>1. map1[i].v_c' = [map1[i].v_c EXCEPT ![j] = [v |-> PCR1!isPrime(PCR1!in(i), PCR1!v_p(i), j), r |-> 0]] BY <3>1, <5>1 DEF PCR1!CtxMap, PCR1!CtxType
          <6>2. map1[i].v_c[j]' = [v |-> PCR1!isPrime(PCR1!in(i), PCR1!v_p(i), j), r |-> 0] BY <3>3, <5>2, <6>1 DEF PCR1!CtxType, PCR1!VarC
          <6>3. PCR1!isPrime(PCR1!in(i), PCR1!v_p(i), j) \in VarCType1 BY Fun_Assumptions DEF VarCType1
          <6> QED
             BY <3>3, <5>2, <6>1, <6>3 DEF PCR1!CtxType, PCR1!VarC             
        <5>5. map1[i]' \in PCR1!CtxType BY <3>1, <3>2, <5>1, <5>3, <5>4 DEF PCR1!CtxMap, PCR1!CtxType, PCR1!VarC, PCR1!VarP         
        <5> QED 
          BY <3>1, <5>1, <5>5 DEF PCR1!CtxType, PCR1!CtxMap         
      <4> QED BY <4>1, <4>2 DEF TypeInv                  
    <3>D. CASE /\ PCR1!Running(i) 
               /\ PCR1!R(i) 
      <4>1. N' \in InType1 BY <2>0, <3>0 DEF TypeInv, Next1     
      <4>2. map1' \in PCR1!CtxMap 
        <5>1. PICK j \in PCR1!Iterator(i) : 
        ...
        <5>3. map1[i].ret' \in VarRType1
          <6>1. map1[i].ret' = PCR1!sum(map1[i].ret, PCR1!v_c(i)[j].v) BY <3>1, <5>1 DEF PCR1!CtxMap, PCR1!CtxType
          <6>2. PCR1!sum(map1[i].ret, PCR1!v_c(i)[j].v) \in VarRType1 BY Fun_Assumptions DEF VarRType1
          <6> QED BY <6>1, <6>2
        <5>4. map1[i].v_c' \in PCR1!VarC
          ...
          <6>3. map1[i].v_c[j].r \in Nat BY <3>3, <5>2 DEF PCR1!CtxMap, PCR1!CtxType, PCR1!VarC
          <6>4. map1[i].v_c[j].r' \in Nat BY <6>2, <6>3
          <6> QED
            BY <3>3, <5>2, <6>1, <6>4 DEF PCR1!CtxType, PCR1!VarC
        <5>5. map1[i].ste' \in PCR1!States
          <6>1. map1[i].ste' = IF PCR1!CDone(i, j) THEN PCR1!END ELSE map1[i].ste BY <3>1, <5>1 DEF PCR1!CtxMap, PCR1!CtxType
          ...
          <6>B. CASE ~ PCR1!CDone(i, j) 
            ...
            <7> QED BY <7>1, <7>2          
          <6> QED BY <6>A, <6>B
        <5>6. map1[i]' \in PCR1!CtxType BY <3>1, <3>2, <5>1, <5>3, <5>4, <5>5 DEF PCR1!CtxMap, PCR1!CtxType, PCR1!VarC 
        <5> QED 
          BY <3>1, <5>1, <5>6 DEF PCR1!CtxType, PCR1!CtxMap
      <4> QED BY <4>1, <4>2 DEF TypeInv                      
    <3>E. CASE /\ PCR1!Running(i) 
               /\ PCR1!Quit(i)  
      <4>1. N' \in InType1 BY <2>0, <3>0 DEF TypeInv, Next1     
      <4>2. map1' \in PCR1!CtxMap                                    
      ...
      <4> QED 
        BY <4>1, <4>2 DEF TypeInv                                                              
    <3> QED
      BY <2>1, <3>A, <3>B, <3>C, <3>D, <3>E DEF Next1, PCR1!Next
  <2>2. CASE Done
    <3>1. /\ N'    \in InType1
